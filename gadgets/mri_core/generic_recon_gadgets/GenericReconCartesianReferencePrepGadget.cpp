...
namespace Gadgetron {
        ...
        : BaseClass(context, properties)
    {
        ...
        ref_prepared_.resize(num_encoding_spaces_, false);
        ...
        for (size_t e = 0; e < h.encoding.size(); e++)
        {
            ...
            GDEBUG_CONDITION_STREAM(verbose, "---> Encoding space : " << e << " <---");
            GDEBUG_CONDITION_STREAM(verbose, "Encoding matrix size: " << e_space.matrix_size.x << " " << e_space.matrix_size.y << " " << e_space.matrix_size.z);
            GDEBUG_CONDITION_STREAM(verbose, "Encoding field_of_view : " << e_space.field_of_view_mm.x << " " << e_space.field_of_view_mm.y << " " << e_space.field_of_view_mm.z);
            GDEBUG_CONDITION_STREAM(verbose, "Recon matrix size : " << r_space.matrix_size.x << " " << r_space.matrix_size.y << " " << r_space.matrix_size.z);
            GDEBUG_CONDITION_STREAM(verbose, "Recon field_of_view :  " << r_space.field_of_view_mm.x << " " << r_space.field_of_view_mm.y << " " << r_space.field_of_view_mm.z);
            ...
            else
            {
                ...
                GDEBUG_CONDITION_STREAM(verbose, "acceFactorE1 is " << p_imaging.acceleration_factor.kspace_encoding_step_1);
                GDEBUG_CONDITION_STREAM(verbose, "acceFactorE2 is " << p_imaging.acceleration_factor.kspace_encoding_step_2);
    ...
    void GenericReconCartesianReferencePrepGadget::process(Core::InputChannel<mrd::ReconData> &in, Core::OutputChannel &out)
    {
        for (auto m1 : in)
        {
