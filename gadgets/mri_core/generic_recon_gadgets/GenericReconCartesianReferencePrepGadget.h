...
namespace Gadgetron {
    ...
    public:
        ...
        GenericReconCartesianReferencePrepGadget(const Core::Context &context, const Core::GadgetProperties &properties);
        ...
        NODE_PROPERTY(average_all_ref_N, bool, "Whether to average all N for ref generation", true);
        /// whether to average all S for ref generation
        NODE_PROPERTY(average_all_ref_S, bool, "Whether to average all S for ref generation", false);

        /// pick specific N or S for ref, these options overwrites average_all_ref_N and average_all_ref_S
        NODE_PROPERTY(N_for_ref, int, "If N_for_ref >=0, this N will be used for ref preparation", -1);
        NODE_PROPERTY(S_for_ref, int, "If S_for_ref >=0, this S will be used for ref preparation", -1);

        /// some reconstruction will benefit to fill back the ref data into data array for the embedded mode
        NODE_PROPERTY(ref_fill_into_data_embedded, bool, "If true and calibration is in embedded mode, fill the full sampled data from ref array into the data array", false);
        ...
        NODE_PROPERTY(prepare_ref_always, bool, "Whether to prepare ref for every incoming ReconData", true);
